...
class Bot {
    ...
    update(data) {
        ...
        this.updatePlayerData(command, data);
    }
    ...
    updatePlayerData(playerId, data) {
        ...
        if(command === 'living_cells') {
            this.state.players[playerId] = { living_cells: parseInt(value, 10) };
            return;
        }

        console.error(`Received unknown player update command "${ command }"`);
    }
